...
export const Topbar = ({ onMenuToggle, onCommandPaletteOpen }: TopbarProps) => {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/85 p-3 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/85">
      <div className="flex flex-wrap items-center gap-2 lg:gap-3">
        ...
        <div className="order-3 flex w-full gap-2 lg:order-none lg:w-auto lg:flex-1">
          <input
          ...
          />
          <button className="btn shrink-0" onClick={onCommandPaletteOpen}>
            ⌘K Quick Ops
          </button>
        </div>
        ...
        <select
          className="input w-full sm:w-52 lg:w-48"
          value={state.role}
          onChange={(e) =>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div
            className={`hidden rounded-lg px-2 py-1 text-xs font-semibold lg:block ${readinessTone}`}
          >
            Readiness Online
          </div>
          <div className="rounded-lg bg-rose-100 px-3 py-2 text-sm font-semibold text-rose-700 dark:bg-rose-900/50 dark:text-rose-200">
            Alerts: {notifCount}
          </div>
        </div>
      </div>
